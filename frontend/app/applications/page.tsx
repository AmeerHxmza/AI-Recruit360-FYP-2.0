...
export default function ApplicationsPage() {
  ...
  React.useEffect(() => {
    let isMounted = true;
    getApplicationsAction({ stage: stageFilter, search: searchQuery }).then((res) => {
      if (!isMounted) return;
      if (res.success && res.data) {
        setApplications(res.data);
      } else {
  ...
  }, [stageFilter, searchQuery, organization?.id]);
  ...
  return (
    <ApplicationShell pageBreadcrumb={[organization?.name || "AI-Recruit360", "Applications"]}>
      <PageHeader
      ...
      <Section title="Active Applications Lifecycle">
        {loading ? (
          <div className="p-12 text-center rounded-xl border border-[#242932] bg-[#12151A] space-y-4">
            ...
            <p className="text-xs text-[#A7AFBC] font-mono">Loading applications from database...</p>
          </div>
          ...
          <div className="p-12 sm:p-16 text-center rounded-2xl border border-[#242932] bg-[#0D0F12] space-y-5">
            ...
            <div className="max-w-md mx-auto space-y-2">
              <h3 className="text-lg font-bold font-display text-[#F5F7FA]">
                {searchQuery ? "No matching applications" : "No pipeline applications yet"}
              </h3>
