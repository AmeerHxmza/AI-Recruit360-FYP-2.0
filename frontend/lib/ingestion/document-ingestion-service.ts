...
import { getCurrentUser, getCurrentRole, getOrganizationContext } from "@/lib/auth/session";
...
import { DocumentIngestionParams, DocumentIngestionResult, CandidateDocumentRow } from "./types";
import {
...
export async function ingestCandidateDocument(
  ...
  if (typeof window !== "undefined") {
    throw new DocumentProcessingError("Ingestion service can only be executed on the server.");
  }

  const { organizationId, candidateId, applicationId, documentType = "resume", fileName, fileType, fileBuffer } = params;
  ...
  const storagePath = `${organizationId}/${candidateId}/${sanitizedFileName}`;

  // 3. Multi-Tenant Authorization & RBAC Checks
  const user = await getCurrentUser();
  if (!user) {
    throw new AuthError("Authentication required to ingest candidate document.");
  }

  const orgCtx = await getOrganizationContext(organizationId);
  if (!orgCtx || orgCtx.organization.id !== organizationId) {
    throw new ForbiddenError("Access denied for requested organization context.");
  }

  const role = await getCurrentRole(organizationId);
  if (!canManageDocuments(role)) {
    throw new ForbiddenError("You do not have RBAC permission to upload candidate documents.");
  }
  ...
  if (!candidate) {
    throw new NotFoundError("Target candidate profile not found in your organization workspace.");
  }
  ...
  if (storageError) {
    console.error("[DocumentIngestion] Storage upload failed:", storageError.message);
    throw new DocumentStorageError(`Failed to upload file to storage: ${storageError.message}`);
  }
  ...
  if (existingDoc) {
    ...
    if (updateError || !updatedDoc) {
      throw new DocumentProcessingError("Failed to update candidate document processing status.");
    }
    ...
    if (insertError || !insertedDoc) {
      throw new DocumentProcessingError("Failed to create candidate document record.");
    }
  ...
  try {
    const parser = getParserForDocument(fileType, sanitizedFileName, registeredParsers);
    ...
    if (finalError || !finalDoc) {
      throw new DocumentProcessingError("Failed to update processed document metadata.");
    }

    // 9. Record Safe AI Activity Log
    try {
      await logAiActivity({
        organization_id: organizationId,
        ...
        metadata: {
          ...
          file_size: fileBuffer.length,
          document_type: documentType,
          parser_version: rawResult.parserVersion,
          extracted_text_length: normalizedText.length,
        },
      });
    } catch {
      // Activity log failures should not block successful ingestion
    }
