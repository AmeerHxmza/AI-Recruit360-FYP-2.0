    ...
    const { result } = await measurePerformance(
      ...
      async () => {
        ...
        let query = supabase
          .from("candidates")
          .select(
            "id, organization_id, full_name, email, phone, location, linkedin_url, portfolio_url, created_at, updated_at",
            { count: "exact" },
          )
  ...
  if (error || !data) {
    if (
      ...
      throw new DatabaseError(
        "A candidate with this email address already exists in your workspace.",
      );
    }
    throw new DatabaseError(
...
export async function updateCandidate(
  ...
  if (input.full_name !== undefined) {
    if (!input.full_name.trim())
      throw new DatabaseError("Full name cannot be empty.");
  ...
  if (input.phone !== undefined)
    updatePayload.phone = input.phone.trim() || null;
  if (input.location !== undefined)
    updatePayload.location = input.location.trim() || null;
  if (input.linkedin_url !== undefined)
    updatePayload.linkedin_url = input.linkedin_url.trim() || null;
  if (input.portfolio_url !== undefined)
    updatePayload.portfolio_url = input.portfolio_url.trim() || null;
...
export async function getCandidateDocuments(
  ...
  return await (async () => {
    ...
    const { data: docs, error } = await supabase
      ...
      .select(
        "id, organization_id, candidate_id, application_id, document_type, storage_path, original_filename, mime_type, file_size, extraction_status, created_at, updated_at",
      )
      .eq("organization_id", orgId)
      .eq("candidate_id", candidateId)
      .order("created_at", { ascending: false });
