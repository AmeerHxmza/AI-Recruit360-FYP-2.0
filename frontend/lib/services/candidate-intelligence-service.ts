...
export async function getCandidateIntelligence(
  ...
  return await (async () => {
    ...
    try {
      ...
      const { data: apps, error: appError } = await applicationQuery;
      ...
      const evalRes = {
        data: getFirst(pipelineData.final_evaluations) || null,
      };

      if (cvRes.data) {
        const ms = Array.isArray(cvRes.data.matched_skills)
          ? (cvRes.data.matched_skills as string[])
          : [];
        const mss = Array.isArray(cvRes.data.missing_skills)
          ? (cvRes.data.missing_skills as string[])
          : [];
        let ev = "";
      ...
      if (intRes.data) {
        ...
        const validTech = responses
          .filter((r) => r.technical_score != null)
          .map((r) => Number(r.technical_score));
        const validComm = responses
          .filter((r) => r.communication_score != null)
          .map((r) => Number(r.communication_score));
        const validRel = responses
        ...
        const avgTech =
          validTech.length > 0
            ? Math.round(
                validTech.reduce((a, b) => a + b, 0) / validTech.length,
              )
            : null;
        const avgComm =
          validComm.length > 0
            ? Math.round(
                validComm.reduce((a, b) => a + b, 0) / validComm.length,
              )
            : null;
        const avgRel =
          validRel.length > 0
            ? Math.round(validRel.reduce((a, b) => a + b, 0) / validRel.length)
            : null;

        let calculatedOverall: number | null =
          intRes.data.overall_score != null
            ? Number(intRes.data.overall_score)
            : null;
        if (
          calculatedOverall == null &&
          evalRes.data?.interview_score != null
        ) {
        ...
        if (
          ...
          calculatedOverall = Math.round(
            avgTech * 0.4 + avgComm * 0.3 + avgRel * 0.3,
          );
        }
        ...
        const isCompleted = intRes.data.status === "completed";
        ...
        result.interview = {
          ...
          feedback:
            fb ||
            (isCompleted
              ? "AI technical interview completed and evaluated."
              : null),
      ...
      if (evalRes.data) {
        let fEv = "";
        if (typeof evalRes.data.evidence === "string")
          fEv = evalRes.data.evidence;
        else if (evalRes.data.evidence)
          fEv = JSON.stringify(evalRes.data.evidence);
        ...
        result.finalEvaluation = {
          ...
          evidence: fEv,
          strengths: Array.isArray(evalRes.data.strengths)
            ? (evalRes.data.strengths as string[])
            : [],
          weaknesses: Array.isArray(evalRes.data.weaknesses)
            ? (evalRes.data.weaknesses as string[])
            : [],
      ...
      throw new DatabaseError(
        "Could not retrieve candidate intelligence data.",
      );
    }
  })();
}
