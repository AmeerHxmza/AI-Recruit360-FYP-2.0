...
function JsonEvidenceRenderer({
  ...
  }
  {
        ...
        <ul className="space-y-1.5 mt-2">
          {parsed.map((item, idx) => {
            ...
            if (typeof item === "object" && item !== null) {
              const text =
                item.evidence_quote || item.requirement || JSON.stringify(item);
              return (
                <li
                  key={idx}
                  className={`text-xs text-text-secondary flex gap-2 items-start ${isItalic ? "italic" : ""}`}
                >
                  <Check className="w-3 h-3 text-action-blue shrink-0 mt-0.5" />
    ...
    if (typeof parsed === "object" && parsed !== null) {
      return (
        <p
          className={`text-xs text-text-secondary leading-relaxed ${isItalic ? "italic" : ""}`}
        >
...
export function CandidateIntelligencePanel({
  intelligence,
  onStatusChange,
}: CandidateIntelligencePanelProps) {
    ...
    return (
      <Card className="p-6 border-border bg-surface space-y-3">
        ...
        <p className="text-xs text-text-secondary leading-relaxed">
          Candidate has not applied for any active jobs yet. No AI intelligence
          available.
        </p>
      </Card>
    );
  ...
  const { application, cvScreening, assessment, interview, finalEvaluation } =
    intelligence;
  ...
  return (
    <div className="space-y-8">
      {/* 1. Header Summary */}
      <Card className="p-6 border-border bg-surface shadow-md relative overflow-hidden">
        <div className="flex flex-col md:flex-row gap-6 justify-between md:items-center">
          <div className="space-y-1">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-4xl sm:text-5xl font-extrabold font-display text-action-blue">
          ...
          <div className="grid grid-cols-3 gap-4 sm:gap-6 border-t md:border-t-0 md:border-l border-border pt-4 md:pt-0 md:pl-6">
            <div className="space-y-1">
              <span className="text-xs text-text-secondary uppercase tracking-wider font-mono">
                CV Match (40%)
              </span>
              <div className="text-lg sm:text-xl font-bold text-text-primary font-mono">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* 2. CV Screening */}
          <Card className="p-6 border-border bg-surface space-y-4 shadow-sm">
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <span className="text-xs text-success flex items-center gap-1 uppercase font-semibold">
                      <CheckCircle2 className="h-3 w-3" /> Matched Skills
                    </span>
                    <div className="flex flex-wrap gap-1.5">
                      {cvScreening.matchedSkills.length > 0 ? (
                        cvScreening.matchedSkills.map((s, i) => (
                          <Badge
                            key={i}
                            variant="outline"
                  ...
                  <div className="space-y-2">
                    <span className="text-xs text-danger flex items-center gap-1 uppercase font-semibold">
                      <XCircle className="h-3 w-3" /> Missing / Weak Skills
                    </span>
                    <div className="flex flex-wrap gap-1.5">
                      {cvScreening.missingSkills.length > 0 ? (
                        cvScreening.missingSkills.map((s, i) => (
                          <Badge
                            key={i}
                            variant="outline"
                ...
                <div className="space-y-2 bg-hover/50 p-4 rounded-xl border border-border">
                  <h4 className="text-xs font-bold text-text-primary uppercase tracking-wider">
                    AI Recommendation & Evidence
                  </h4>
                  <Badge
                    variant={
                      cvScreening.matchScore >= 70 ? "success" : "danger"
                    }
                    className="mb-2 uppercase text-[9px] font-bold"
                  >
          ...
          <Card className="p-6 border-border bg-surface space-y-4 shadow-sm">
            ...
            {assessment ? (
              <div className="space-y-4">
                <div className="flex items-center gap-6 text-sm font-medium text-text-secondary mb-4">
                  <span className="flex items-center gap-1 text-success">
                    <Check className="w-4 h-4" /> {assessment.correctAnswers}{" "}
                    Correct
                  </span>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2 bg-hover/50 p-4 rounded-xl border border-border">
                  <h4 className="text-xs font-bold text-text-primary uppercase tracking-wider flex items-center gap-2">
                    <Sparkles className="w-3 h-3 text-action-blue" /> AI Overall
                    Feedback
                  </h4>
                  <p className="text-xs text-text-secondary leading-relaxed">
                    {interview.feedback ||
                      "Feedback will be generated upon interview completion."}
                  </p>
                </div>
                ...
                {interview.responses && interview.responses.length > 0 && (
                  <div className="space-y-3 pt-2">
                    ...
                    {interview.responses.map((r, idx) => (
                      <div
                        key={idx}
                        className="space-y-2 bg-surface border border-border p-4 rounded-lg"
                      >
                        <div className="flex gap-3">
                          <MessageSquare className="w-4 h-4 text-action-blue shrink-0 mt-0.5" />
                          <p className="text-xs text-text-primary font-medium leading-relaxed">
                            {r.questionText}
                          </p>
                        </div>
        ...
        <div className="space-y-6">
          {/* 5. Final Evaluation */}
          <Card className="p-6 border-border bg-surface space-y-5 shadow-sm relative">
            <div className="flex items-center gap-2 border-b border-border pb-3">
            ...
            {finalEvaluation ? (
              <div className="space-y-5">
                <div className="space-y-2">
                  ...
                  <Badge
                    variant={
                      finalEvaluation.recommendation === "strong_hire"
                        ? "success"
                        : finalEvaluation.recommendation === "hire"
                          ? "ai"
                          : finalEvaluation.recommendation === "review"
                            ? "warning"
                            : "danger"
                    }
                ...
                <div className="space-y-2">
                  ...
                  <ul className="space-y-1">
                    {finalEvaluation.strengths.map((s, i) => (
                      <li
                        key={i}
                        className="text-xs text-text-primary flex gap-2 items-start"
                      >
          ...
          <Card className="p-6 border-border bg-surface">
            <h3 className="text-xs font-semibold text-text-primary uppercase tracking-wider mb-4 border-b border-border pb-3">
              Application Timeline
            </h3>
            <div className="space-y-4 relative before:absolute before:inset-0 before:ml-[13px] before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-white/[0.12] before:to-transparent">
              <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                <div className="flex items-center justify-center w-7 h-7 rounded-full border border-border bg-hover text-text-secondary shrink-0 z-10 font-bold text-xs">
                  1
                </div>
                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] flex flex-col md:text-right pl-4 md:pl-0 md:pr-4">
                  ...
                  <span className="text-xs text-text-muted">
                    {formatTimelineDate(application.appliedAt)}
                  </span>
                </div>
              </div>
              ...
              {cvScreening && (
                <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  <div className="flex items-center justify-center w-7 h-7 rounded-full border border-border bg-action-blue text-primary-foreground shrink-0 z-10">
                  ...
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] flex flex-col pl-4 md:pl-4">
                    ...
                    <span className="text-xs text-text-muted">
                      {formatTimelineDate(cvScreening.createdAt)}
                    </span>
                  </div>
                </div>
              )}
              ...
              {assessment && (
                <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  <div className="flex items-center justify-center w-7 h-7 rounded-full border border-border bg-warning text-primary-foreground shrink-0 z-10">
                    <Check className="w-3 h-3" />
                  </div>
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] flex flex-col md:text-right pl-4 md:pl-0 md:pr-4">
                    <span className="text-xs text-text-secondary uppercase font-bold">
              ...
              {interview && (
                <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  <div className="flex items-center justify-center w-7 h-7 rounded-full border border-border bg-success text-primary-foreground shrink-0 z-10">
                    <Check className="w-3 h-3" />
                  </div>
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] flex flex-col pl-4 md:pl-4">
                    <span className="text-xs text-text-secondary uppercase font-bold">
              ...
              {finalEvaluation && (
                <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  ...
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] flex flex-col md:text-right pl-4 md:pl-0 md:pr-4">
                    ...
                    <span className="text-xs text-text-muted">
                      {formatTimelineDate(finalEvaluation.createdAt)}
                    </span>
                  </div>
                </div>
              )}
          ...
          {/* Recruiter Decision */}
          <Card className="p-6 border-border bg-surface space-y-4 shadow-sm sticky top-24">
            <h3 className="text-xs font-semibold text-text-primary uppercase tracking-wider border-b border-border pb-3">
              Recruiter Decision
            </h3>
            ...
            <div className="space-y-3 pt-2">
              ...
              <Button
                variant="outline"
                className="w-full justify-between hover:bg-blue-500/10 hover:text-action-blue hover:border-blue-500/30"
