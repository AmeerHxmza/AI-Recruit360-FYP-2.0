...
export function InterviewsClientView({
  ...
  const [interviews, setInterviews] =
    React.useState<InterviewItemWithDetails[]>(initialInterviews);
  ...
  const metrics = React.useMemo(() => {
    ...
    const aiAdaptive = interviews.filter(
      (i) => i.interview_type === "ai_adaptive",
    ).length;
        ...
        {loading ? (
          <div className="p-12 text-center rounded-xl border border-border bg-surface space-y-4">
            ...
            <p className="text-xs text-text-secondary font-mono">
              Updating interview rooms...
            </p>
          </div>
        ) : interviews.length === 0 ? (
          <div className="p-12 sm:p-16 text-center rounded-2xl border border-border bg-surface space-y-5">
            <div className="h-14 w-14 rounded-2xl bg-hover border border-border text-action-blue mx-auto flex items-center justify-center shadow-sm">
            ...
            <div className="max-w-md mx-auto space-y-2">
              ...
              <p className="text-xs text-text-secondary leading-relaxed">
                Candidates who pass the technical assessment will automatically
                be invited to an AI interview.
              </p>
            </div>
          </div>
        ) : (
                  ...
                  >
                    <TableCell className="py-4">
                      <div className="flex items-center gap-3">
                        <Avatar
                          fallback={getInitials(item.candidateName)}
                          size="sm"
                          status="online"
                        />
                        <div className="flex flex-col">
                          <span
                            ...
                            className="font-semibold text-text-primary text-xs hover:text-action-blue hover:underline"
                          >
                            {item.candidateName}
                          </span>
                    ...
                    <TableCell>
                      <span className="rounded bg-hover px-2 py-0.5 border border-border text-xs font-mono text-action-blue">
                      ...
                      item.overall_score !== undefined ? (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-text-secondary">
                            ...
                            {item.total_questions || 5} Qs)
                          </span>
                        </div>
                      ) : (
                        <span className="text-action-blue">
                          {item.questions_answered || 0} /{" "}
                          {item.total_questions || 5} Questions
                        </span>
                      )}
                    </TableCell>
                      ...
                      >
                        {item.candidateId && (
                          <Button
                            ...
                            className="text-xs text-text-secondary hover:text-action-blue h-7 px-2"
                          >
                            Profile
                          </Button>
                        )}
                        <Button
                          ...
                          size="icon"
                          onClick={() => router.push(`/interviews/${item.id}`)}
                          className="h-7 w-7 text-text-secondary hover:text-action-blue"
